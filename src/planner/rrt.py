...
def trilinear_interpolation(voxel_grid: np.ndarray, point: np.ndarray) -> np.ndarray:
    ...
    # Clamp the upper index to grid size - 1 to handle points exactly on the
    # boundary (e.g. x == H-1): in that case x0 == x1 and dx == 0, so the
    # interpolation correctly returns the boundary value.
    x0 = int(x)
    y0 = int(y)
    z0 = int(z)
    x1 = min(x0 + 1, H - 1)
