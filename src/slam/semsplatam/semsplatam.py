...
class SemSplatam(SplatamOurs):
    def __init__(self,
       ...
       '''
        ...
        # Open-vocabulary CLIP embeddings (optional)
        self.clip_extractor = None
        self.clip_embed_dim = 0
        if getattr(self.slam_cfg, 'use_clip_embeddings', False):
            from src.slam.semsplatam.modified_ver.semantic.open_vocab_extractor import CLIPFeatureExtractor
            self.clip_embed_dim = getattr(self.slam_cfg, 'clip_embed_dim', 64)
            clip_device = getattr(self.slam_cfg, 'clip_device', str(self.device))
            self.clip_extractor = CLIPFeatureExtractor(
                model_name=getattr(self.slam_cfg, 'clip_model', 'ViT-B-32'),
                pretrained=getattr(self.slam_cfg, 'clip_pretrained', 'openai'),
                device=clip_device,
                embed_dim=self.clip_embed_dim,
            )
            info_printer(f"Open-vocabulary CLIP initialized: embed_dim={self.clip_embed_dim}", step=0, stage="Initialization")
    ...
    def render(self, c2w: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Returns:
            ...
            rastered_depth: (1,H,W) # render depth
            valid_depth_mask: (1,H,W) valid rendering mask
            seen: (H,W) mask of visible gaussians
        '''
        cam = self.cam
    ...
    def online_recon_step(self, 
        ...
        Args:
            ...
            keyframes_extra_subdir: if set, duplicate keyframe RGB dumps under
                ``result_dir/<subdir>/`` (e.g. exploration-only folder from the main loop).
    ...
    def update_gs_map(self, 
        ...
        Args:
            ...
            c2w     : pose. Format: RUB camera-to-world, [4,4]
            force_map_update: run map update if true
            keyframes_extra_subdir: optional extra folder under result_dir to duplicate
                keyframe RGB dumps into.
        '''
        ### get self variables ###
        ...
        # Controlled by main_cfg.slam.save_keyframes (default True).
        # Saved to result_dir/keyframes/ (outside splatam/ subdir).
        # Optional duplicate: result_dir/<keyframes_extra_subdir>/ (e.g. exploration pass only).
        _save_kf = getattr(self.main_cfg.slam, 'save_keyframes', True)
        if _save_kf and (
            not(dont_add_kf) and
            ((time_idx == 0) or ((time_idx+1) % config['keyframe_every'] == 0) or
             (time_idx == num_frames-2)) and
            (not torch.isinf(curr_gt_w2c[-1]).any()) and
            (not torch.isnan(curr_gt_w2c[-1]).any())
        ) or (force_map_update and _save_kf):
            import cv2
            _kf_dirs = [os.path.join(self.main_cfg.dirs.result_dir, "keyframes")]
            if keyframes_extra_subdir:
                _kf_dirs.append(
            ...
            for _kd in _kf_dirs:
                ...
                cv2.imwrite(os.path.join(_kd, _fname), color_bgr)

        # Periodic checkpoint saving is disabled here.
        # Checkpoints are saved via save_checkpoint() called alongside validation.
    ...
    def print_and_save_result(self, eval_dir_suffix="", is_prune_gaussians=False, ignore_first_frame=False):
        ...
        # Save all keyframe RGB images at the end (if enabled)
        if getattr(self.main_cfg.slam, 'save_keyframes', True):
            import cv2
            keyframes_dir = os.path.join(self.main_cfg.dirs.result_dir, "keyframes")
    ...
    def save_checkpoint(self, eval_dir_suffix: str) -> None:
        """Save Gaussian parameters, PLY exports and keyframe indices.

        Unlike the old periodic checkpoint inside ``online_recon_step``,
        this method is called explicitly from the main loop alongside
        validation so that checkpoints are always aligned with eval steps.

        Files saved to ``result_dir/splatam/<eval_dir_suffix>/``:
            * ``params<step>.npz``     – full Gaussian parameter dict
            * ``rgb_GS_<step>.ply``    – RGB-coloured point cloud
            * ``semantic_GS_<step>.ply`` – semantics-coloured point cloud
            * ``keyframe_time_indices<step>.npy`` – keyframe frame indices
        """
        params = self.params
        variables = self.variables
        keyframe_time_indices = self.keyframe_time_indices

        # Each step has its own directory, so we always use index=0 inside it.
        # Files: params0.npz, rgb_GS_0000.ply, semantic_GS_0000.ply,
        #        keyframe_time_indices.npy
        ckpt_dir = os.path.join(self.results_dir, eval_dir_suffix)
        os.makedirs(ckpt_dir, exist_ok=True)

        _idx = 0
        save_params_ckpt(params, variables, ckpt_dir, _idx)
        save_semantic_ply(params, variables, ckpt_dir, _idx, self.n_cls)
    ...
    def eval_semantic_result(self, eval_dir_suffix="", ignore_first_frame=False, save_frames=False, visualizer=None, main_cfg=None):
        """ evaluate rendering results
        ...
        with torch.no_grad():
            if config['use_wandb']:
                ...
                eval_semantic(self, dataset, params, variables, num_frames, eval_dir, sil_thres=config['mapping']['sil_thres'],
                     ...
                     add_new_gaussians=config['mapping']['add_new_gaussians'],
                     eval_every=config['eval_every'], ignore_first_frame=ignore_first_frame, save_frames=save_frames,
                     visualizer=visualizer, main_cfg=main_cfg)
        return
