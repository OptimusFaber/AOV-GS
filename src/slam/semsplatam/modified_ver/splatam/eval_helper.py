...
def eval_semantic(slam_model, dataset, final_params, final_variables, num_frames, eval_dir, sil_thres,
         mapping_iters, add_new_gaussians, wandb_run=None, wandb_save_qual=False, eval_every=1, save_frames=False,
         ignore_first_frame=False, visualizer=None, main_cfg=None):
    print("Evaluating Final Parameters ...")
    miou_g_curr_list = []
    miou_p_curr_list = []
    miou_g_list = []
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        # Initialize Render Variables
        rendervar = transformed_params2rendervar(final_params, transformed_gaussians)
        # Render RGB and depth for visualization (and radius for semantic)
        im, radius, _, = Renderer(raster_settings=curr_data['cam'])(**rendervar)
        seen = radius > 0
        # Rendered depth for visualization when visualizer is enabled
        if visualizer is not None:
            depth_sil_rendervar = transformed_params2depthplussilhouette(final_params, curr_data['w2c'],
                                                                         transformed_gaussians)
            depth_sil, _, _, = Renderer(raster_settings=curr_data['cam'])(**depth_sil_rendervar)
            rastered_depth = depth_sil[0, :, :].to(color.device)
        else:
            rastered_depth = None

        # Optional: show RGB-D + rendered view when enable_vis=1
        if visualizer is not None and main_cfg is not None and rastered_depth is not None:
            max_depth = getattr(main_cfg.visualizer, 'vis_rgbd_max_depth', 10.0)
            rgb = (color.float() / 255.0) if color.max() > 1 else color.float()
            rgb = rgb.to(color.device)
            depth_gt = depth.squeeze().to(color.device)
            if depth_gt.dim() == 3:
                depth_gt = depth_gt[0] if depth_gt.shape[0] == 1 else depth_gt[..., 0]
            im_vis = im.permute(1, 2, 0) if im.dim() == 3 else im
            visualizer.visualize_rgbd_w_render(rgb, depth_gt, im_vis, rastered_depth, max_depth)
