...
class ReplicaDataset(GradSLAMDataset):
    ...
    def get_filepaths(self):
        # data/Replica/*/results/  vs  data/replica_sim_nvs/*/results_habitat/
        if os.path.isdir(os.path.join(self.input_folder, "results_habitat")):
            subdir = "results_habitat"
        else:
            subdir = "results"
        color_paths = natsorted(glob.glob(f"{self.input_folder}/{subdir}/frame*.jpg"))
        depth_paths = natsorted(glob.glob(f"{self.input_folder}/{subdir}/depth*.png"))
