...
def transform_to_frame(params, time_idx, gaussians_grad, camera_grad, rel_w2c=None):
    ...
    if rel_w2c is None:
        ...
        # Explicit w2c (e.g. GT pose during eval): rotation is still needed to transform
        # anisotropic Gaussian orientations into the current camera frame.
        R = rel_w2c[:3, :3]
        cam_rot = matrix_to_quaternion(R.unsqueeze(0))[0]
...
def eval(dataset, final_params, num_frames, eval_dir, sil_thres, 
    ...
    try:
        ...
        if use_world_align:
            print("Skipping ATE: eval uses cross-sequence world alignment (not comparable to SLAM cam params).")
            raise RuntimeError("skip_ate_cross_eval")
        ...
        error_msg = str(e)
        if error_msg == "skip_ate_cross_eval":
            ate_rmse = float("nan")
        else:
            ate_rmse = 100.0
    ...
    ate_title = "N/A" if not np.isfinite(ate_rmse) else f"{ate_rmse * 100:.2f}"
    fig.suptitle(
        "Average PSNR: {:.2f}, Average Depth L1: {:.2f} cm, ATE RMSE: {} cm".format(
            avg_psnr, avg_l1 * 100, ate_title
        ),
        y=1.05,
        fontsize=16,
    )
