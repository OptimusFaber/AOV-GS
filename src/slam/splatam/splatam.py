...
class SplatamOurs(SlamModel):
    ...
    def load_params(self, stage="final"):
        ...
        if checkpoint_time_idx == 0:
            ...
            # print_and_save_result saves under e.g. splatam/exploration_stage_1/ ;
            # eval_result writes plots under splatam/eval_exploration_stage_1/ — users often confuse the two.
            stage_dirs = [stage]
            ...
            for d in stage_dirs:
                ...
                if os.path.isfile(p):
                    ckpt_path = p
                    if d != stage:
                        print(f"Resolved checkpoint: using folder '{d}' (params not found under '{stage}')")
                    break
            if ckpt_path is None:
                tried = [os.path.join(base, d, "params.npz") for d in stage_dirs]
                raise FileNotFoundError(
                    "params.npz not found. Tried:\n  " + "\n  ".join(tried)
                )
        else:
    ...
    def _eval_alignment_kwargs(self):
        ...
        if train_bd == eval_bd:
            return dict(train_first_abs_c2w=None, align_eval_world=False)
        if not hasattr(self.dataset_sample, "poses_absolute"):
            return dict(train_first_abs_c2w=None, align_eval_world=False)
    ...
    def _write_effective_config(self, output_path: str) -> None:
        ...
        with open(output_path, "w", encoding="utf-8") as cfg_f:
            cfg_f.write("# Auto-generated by AOV-GS SplaTAM wrapper.\n")
