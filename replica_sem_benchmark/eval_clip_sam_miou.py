#!/usr/bin/env python3
"""
Обертка над ``scripts/eval_clip_sam_miou.py`` — запуск из ``replica_sem_benchmark/``.

``_PROJ`` = родитель каталога скрипта (``New-Proj``).

Пример::

    python replica_sem_benchmark/eval_clip_sam_miou.py \\
...
_PROJ = _HERE.parent  # New-Proj (каталог над replica_sem_benchmark)
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="JSON: классы на каждый sample_id (см. scripts/eval_clip_sam_miou.py)",
    )
    ap.add_argument(
        "--queries_from_gt",
        action="store_true",
