#!/bin/bash
# Build aov-gs for HPC nodes without sudo (cds2 and similar).
#
# Problem: conda linker (x86_64-conda-linux-gnu-c++) when building tinycudann
# passes -L$CONDA_PREFIX/lib, but libcuda is only in lib/stubs → "cannot find -lcuda".
# Fix: symlink libcuda.so into $CONDA_PREFIX/lib + explicit LDFLAGS/LIBRARY_PATH.
#
# Usage (from AOV-GS root):
#   bash scripts/installation/conda_env/build_sem_cds2.sh
#   bash scripts/installation/conda_env/build_sem_cds2.sh --env-only   # env already exists
#   bash scripts/installation/conda_env/build_sem_cds2.sh --fix-tcnn    # CUDA extensions only
#
# Before launching on Ampere/Ada GPUs (optional):
#   export TCNN_CUDA_ARCHITECTURES="75;80;86"   # or 89 for Ada
#   export TORCH_CUDA_ARCH_LIST="8.0;8.6"

# Without -u: conda deactivate.d (gcc_linux-64) fails on unbound _CONDA_PYTHON_SYSCONFIGDATA_NAME_USED
set -eo pipefail
...
usage() {
  ...
  echo ""
  echo "Flags:"
  echo "  --env-only     do not create conda env (${ENV_NAME} must already exist)"
  echo "  --fix-tcnn     tinycudann + diff-gaussian only (env already partially built)"
  echo "  --skip-habitat skip habitat-sim build"
...
# libcuda linking for conda gcc (cds2 fix).
setup_cds2_cuda_linking() {
  ...
  if [[ ! -f "${stubs}/libcuda.so" ]]; then
    echo "ERROR: ${stubs}/libcuda.so not found."
  ...
  if [[ -e "${system_cuda}/libcuda.so" || -e "${system_cuda}/libcuda.so.1" ]]; then
    ...
    echo "✓ added system path: ${system_cuda}"
  fi
}
...
export_conda_cuda_build_env() {
  ...
  # By default include Volta (70) + Turing/Ampere. Set Ada (89) explicitly via env var.
...
install_cuda_extensions() {
  echo "=== Installing tiny-cuda-nn and diff-gaussian-rasterization (cds2 linking) ==="
  export_conda_cuda_build_env
...
if [[ "${FIX_TCNN_ONLY}" -eq 1 ]]; then
  ...
  echo "=== --fix-tcnn mode: rebuilding CUDA extensions in ${ENV_NAME} ==="
  install_cuda_extensions
  verify_install
  echo ""
  echo "=== Done (--fix-tcnn) ==="
...
# --- full installation ---
echo "=== Building aov-gs (cds2 / HPC, no sudo) ==="
echo "Repo root: ${ROOT}"
...
# Pin 11.7: on cds2 cuda-driver-dev_linux-64 13.x is sometimes pulled in
...
for path in cuda/std thrust cub; do
  if [[ ! -d "${CONDA_PREFIX}/include/${path}" ]]; then
    echo "ERROR: ${CONDA_PREFIX}/include/${path} not found"
    exit 1
  fi
  echo "✓ ${path}"
done
...
if [[ "${SKIP_HABITAT}" -eq 0 ]]; then
  ...
  if [[ -d "${HABITAT_SRC}/.git" ]] || [[ -f "${HABITAT_SRC}/setup.py" ]] || \
    ...
    echo "Found ${HABITAT_SRC}: clone skipped."
  else
  ...
  echo "=== habitat-sim skipped (--skip-habitat) ==="
fi

install_cuda_extensions

echo "=== Remaining pip dependencies ==="
#
# Important: environment is Python 3.8. Newer transformers/tokenizers require Python>=3.9,
# and tokenizers starts building from sdist and fails on backend deps.
# So we pin compatible versions (latest stable for py3.8).
...
echo ""
echo "=== SUCCESS (build_sem_cds2.sh) ==="
echo "  conda activate ${ENV_NAME}"
echo ""
echo "If only tinycudann failed in an old env:"
