#!/usr/bin/env python3
"""Generate ActiveOpenSem / Geom / Hybrid configs for Replica scenes."""
...
ACTIVE_OPEN_SEM = '''\
"""
ActiveOpenSem ({scene}) — SplaTAM + active_gs + SAM/CLIP feature extraction.
"""
...
def main() -> None:
    for scene, params in SCENES.items():
        ...
        base = ACTIVE_OPEN_SEM.format(scene=scene, bbox=params["bbox"], xy=params["xy"])
        (d / "ActiveOpenSem.py").write_text(base, encoding="utf-8")
        (d / "ActiveOpenSemGeom.py").write_text(GEOM, encoding="utf-8")
        (d / "ActiveOpenSemHybrid.py").write_text(HYBRID, encoding="utf-8")
